...
def new_day():  # determines if this a new day file needs to be created
    if os.path.exists(datetime.datetime.now().strftime("%B%d%Y") + ".csv"):
        cls()
        main(4)
    try:
    ...
    for item in months:
        if os.path.exists('Archive\\' + item + '\\' + 'monthlystats.txt'):
            ...
            for data in stats:
                ...
                'Archive\\' + datetime.datetime.now().strftime("%Y") + '\\' + item + '\\' + 'monthlystats.txt'):
...
def add_steps(kind, steps_current, steps_add):  # add steps to stats file
    # sometimes the watch gets reset
    if kind == 'sync':
        if int(steps_current) > int(steps_add):
            confirm = ynbox(
                " steps already logged: {0} \n steps on watch: {1} \n should I add the values? ".format(steps_current,
                                                                                                        steps_add))
            if confirm == True:
...
def main(error):  # main loop
    ...
    if command == 'sync pinetime':
        ...
        add_steps('sync', steps_num, step_val)  # kind, steps_current, steps_add
    if command == 'exercise':
    ...
    if command == 'log':  # show the food log so far as well as the current weigh and other useful information while logging.
        ...
        if command == 'add steps':  # change steps manually
            ...
            add_steps('manual', steps, step_val)  # kind, steps_current, steps_add
